...
export default function Coach() {
  ...
  // NEW AUTO-FILL LOGGER: Prefills data passed from the UI
  const openLogger = (exerciseName: string, suggestedWeight?: number | string, suggestedReps?: number | string, suggestedSets?: number | string) => {
    ...
    // Priority: 1. Draft -> 2. Overload Target from UI -> 3. History
    setLogWeight(draft?.weight !== undefined ? draft.weight : (suggestedWeight !== undefined ? suggestedWeight : (lastLift?.weight || '')));
    setLogReps(draft?.reps !== undefined ? draft.reps : (suggestedReps !== undefined ? suggestedReps : (lastLift?.reps || '')));
    setLogSets(draft?.sets !== undefined ? draft.sets : (suggestedSets !== undefined ? suggestedSets : 1));
    
    setLogModalOpen(true);
  };

  // Pass targets down when checkbox is clicked
    ...
    if (history.length === 0) {
      return (
        <Typography variant="body2" sx={{ color: '#8a8a9a', fontStyle: 'italic', p: 2, bgcolor: 'rgba(0,0,0,0.2)', borderRadius: 2 }}>
          {minReps && maxReps ? `No previous logs found matching this ${minReps}-${maxReps} rep range. Time to set a baseline!` : `No previous logs found for this exercise.`}
        </Typography>
      );
    }
          ...
          <Paper sx={{ p: 0, borderRadius: 4, bgcolor: 'rgba(176, 106, 255, 0.05)', border: '1px solid rgba(176, 106, 255, 0.2)', overflow: 'hidden' }}>
            ...
            <Box sx={{ p: 3 }}>
                  ...
                  <Paper key={idx} onClick={() => toggleCellExpand(ex.name)} sx={{ 
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: isExpanded ? 1 : 0 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Checkbox checked={isDone} onClick={(e) => handleCheckboxClick(e, ex.name, isDone, 0, ex.reps, 1)} sx={{ color: '#b06aff', '&.Mui-checked': { color: '#00e096' }, p: 0 }} />
                        <Box 
              ...
              {workoutData.mainBlock.map((ex, idx) => {
                ...
                const parsedSets = ex.sets || (ex.setsReps ? parseInt(ex.setsReps.split('x')[0]?.trim() || '3') : 3);
                let parsedRepsRange = `${ex.repsMin}-${ex.repsMax}`;
                if (!ex.repsMin || !ex.repsMax) {
                  parsedRepsRange = ex.setsReps ? ex.setsReps.split('x')[1]?.trim() : '10';
                }

                let relevantHistory = allLiftsDB
                  .filter(l => l.exerciseName.toLowerCase() === ex.name.toLowerCase())
                  .sort((a,b) => b.timestamp - a.timestamp);
                
                if (ex.repsMin && ex.repsMax) {
                  relevantHistory = relevantHistory.filter(l => l.reps >= ex.repsMin! && l.reps <= ex.repsMax!);
                }
                  ...
                  <Paper key={idx} onClick={() => toggleCellExpand(ex.name)} sx={{ 
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: isExpanded ? 1 : 0 }}>
                      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                        <Checkbox checked={isDone} onClick={(e) => handleCheckboxClick(e, ex.name, isDone, suggestedWeightForLogger, ex.repsMax || parsedRepsRange, parsedSets)} sx={{ color: '#b06aff', '&.Mui-checked': { color: '#00e096' }, p: 0, mt: 0.5 }} />
                        
                        <Box 
                          onClick={(e) => { if(isDone) { e.stopPropagation(); openLogger(ex.name, suggestedWeightForLogger, ex.repsMax, parsedSets); } }} 
                          sx={{ display: 'flex', flexDirection: 'column', cursor: isDone ? 'pointer' : 'default', '&:hover': { opacity: isDone ? 0.8 : 1 } }}
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Typography sx={{ fontWeight: 700, fontSize: '1.1rem', textDecoration: isDone ? 'line-through' : 'none' }}>
                          ...
                          <Typography variant="body2" sx={{ color: '#00d4ff', fontWeight: 600, fontSize: '0.85rem', mt: 0.5, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
                            <span><strong style={{color: '#fff'}}>Sets:</strong> {parsedSets}</span> |
                            <span><strong style={{color: '#fff'}}>Reps:</strong> {parsedRepsRange}</span> |
