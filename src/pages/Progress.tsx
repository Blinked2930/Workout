...
export default function Progress() {
  ...
  useEffect(() => { localStorage.setItem('progress_metric', metric); }, [metric]);
      ...
      <Box sx={{ px: { xs: 2, md: 4 }, pt: { xs: 3, md: 5 }, pb: 10, maxWidth: { xs: 480, md: 900 }, mx: 'auto' }}>
          ...
          <Box sx={{ mb: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2 }}>
            <Box sx={{ flex: 1, minWidth: 0 }}>
              {availableEquipments.length > 1 && (
                <>
                  <Typography sx={{ fontSize: '0.72rem', color: 'text.secondary', mb: 1, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Equipment Types Used</Typography>
                  <ToggleButtonGroup
                    value={activeEquipment} exclusive
                    onChange={(_, v) => v && setEquipmentOverride(v)}
                    size="small"
                    sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, '& .MuiToggleButtonGroup-grouped': { border: 0 }, '& .MuiToggleButton-root': { fontWeight: 700, fontSize: '0.78rem', borderRadius: '10px !important', border: '1px solid rgba(255,255,255,0.1) !important', px: 1.5, py: 0.75, '&.Mui-selected': { bgcolor: 'rgba(0,212,255,0.15)', color: '#00d4ff', borderColor: 'rgba(0,212,255,0.4) !important' } } }}
                  >
                    {availableEquipments.map(eq => (
                      <ToggleButton key={eq} value={eq}>{EQUIPMENT_EMOJIS[eq] ?? '⚡'} {eq}</ToggleButton>
                    ))}
                  </ToggleButtonGroup>
                </>
              )}
          ...
          <Box sx={{ mb: 3 }}>
            {!isBW ? (
              <>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2,1fr)', md: 'repeat(2,1fr)' }, gap: 1.5, mb: 1.5 }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2,1fr)', md: 'repeat(2,1fr)' }, gap: 1.5 }}>
                  <Paper sx={{ p: 1.5, borderRadius: 3, textAlign: 'center' }}>
        ...
        {selectedExercise && (
          <Box sx={{ display: 'flex', gap: 1, mb: 2.5, flexWrap: 'wrap' }}>
            {currentMetricsList.map(m => (
              <Chip key={m.value} label={m.label} onClick={() => setMetric(m.value)} sx={{ cursor: 'pointer', fontWeight: 700, bgcolor: metric === m.value ? `${m.color}22` : 'rgba(255,255,255,0.05)', color: metric === m.value ? m.color : 'text.secondary', border: metric === m.value ? `1px solid ${m.color}55` : '1px solid transparent' }} />
            ))}
          </Box>
        )}
            ...
            <Box>
              {chartData.length > 1 && (
                <Paper sx={{ p: 2, borderRadius: 3, mb: 3 }}>
                  <Typography sx={{ fontWeight: 700, mb: 2, fontSize: '0.82rem', color: 'text.secondary' }}>{selectedMetric.label} Trend</Typography>
              ...
              {chartData.length === 1 && (
                <Paper sx={{ p: 3, borderRadius: 3, textAlign: 'center', mb: 3 }}><Typography sx={{ fontSize: '1.5rem', mb: 1 }}>📍</Typography><Typography sx={{ color: 'text.secondary', fontSize: '0.9rem' }}>One session logged — keep going to see your trend!</Typography></Paper>
              )}
            </Box>
                      ...
                      <Paper key={i} onClick={() => handleEditOpen(entry.rawSet)} sx={{ px: 2, py: 1.5, borderRadius: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', '&:hover': { bgcolor: 'rgba(255,255,255,0.03)' } }}>
                        ...
                        <Box sx={{ textAlign: 'right', flexShrink: 0 }}>
                          {isBW ? (
                            ...
                            <><Typography sx={{ fontWeight: 800, fontSize: '0.9rem', color: '#00d4ff' }}>{entry.e1rm > 0 ? entry.e1rm.toFixed(0) : '—'}</Typography><Typography sx={{ fontSize: '0.65rem', color: 'text.secondary' }}>e1RM</Typography></>
                          )}
                        </Box>
                      </Paper>
                    ))}
        ...
        {!selectedExercise && (
          <Paper sx={{ p: 3, borderRadius: 3, textAlign: 'center', mb: 3, bgcolor: 'transparent', border: '1px dashed rgba(255,255,255,0.1)' }}>
            ...
            <Typography sx={{ color: 'text.secondary', fontSize: '0.9rem' }}>Search for a logged exercise above<br />to see your progress dashboard</Typography>
          </Paper>
        )}
